...
async def update_note(note_id: str, data: NoteUpdate, current_user: dict = Depends(get_current_user)):
    col = get_notes_collection()
    try:
        note = col.find_one({"_id": ObjectId(note_id), "user_id": current_user["id"]})
    except Exception:
    ...
    updated = col.find_one({"_id": ObjectId(note_id)})
...
async def delete_note(note_id: str, current_user: dict = Depends(get_current_user)):
    col = get_notes_collection()
    try:
        result = col.delete_one({"_id": ObjectId(note_id), "user_id": current_user["id"]})
    except Exception:
        raise HTTPException(status_code=404, detail="Note not found")
